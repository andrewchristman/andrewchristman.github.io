...
## Git in Action 
The following workflow scenario shows what collaborative version control with Git can look like. An outdoor adventure software company maintains an online help center for its consumer outdoor navigation app. A recent app software update requires updating the online how-to guide in four different areas. Technical writers working for the company each take on one of the areas to complete. Each writer makes a copy of the central Git repository on their local computer. Each then makes a branch to work on their specific update independently. Once complete, the writers push their changes to the central repository for review. The documentation manager approves the changes, and the changes are incorporated into the how-to guide by merging the branches into the main branch. By merging the branches into the main branch, the company can easily incorporate the contributions of multiple technical writers working independently. This workflow results in better documentation for consumers and improved customer satisfaction.    
...
* The free [**Pro Git**](https://git-scm.com/book/en/v2) book by Scott Chacon and Ben Straub provides detailed instructions for how to use Git. 
